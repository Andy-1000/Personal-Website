/*
  Layout audit for /novels.

  Loads novels.html in an off-screen iframe at each viewport size, switches
  ...
  Run it from the browser console on any page of the site:
      ...
      s.src = "novels-audit.js";
      document.head.appendChild(s);
      // then
      await NovelsAudit.run();

  NovelsAudit.run() prints a table and resolves to { pass, fail, failures, results }.
*/
(function (global) {
  "use strict";
  ...
  function run(opts) {
    ...
    return sizes.reduce(function (chain, size) {
      ...
      console.log("%c/novels layout audit", "font-weight:bold;font-size:14px");
  ...
  global.NovelsAudit = { run: run, SIZES: SIZES, SORTS: SORTS };
})(window);
